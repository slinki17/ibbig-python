...
double getScore4D(int *positions, int actualGenes, double *covTen, int noRows, int noCols, int noDepth, int noTime, double alpha){
	...
	int *cols = malloc(noCols * sizeof(int));
	int *depths = malloc(noDepth * sizeof(int));
	int *times = malloc(noTime * sizeof(int));
	
    for (int i = 0; i < actualGenes; i++){
        if (positions[i] < noCols){
            cols[actualCols++] = positions[i];
        }else if (positions[i] >= noCols && positions[i] < noCols + noDepth){
            depths[actualDepth++] = positions[i] - noCols;
        }
		else{
			times[actualTime++] = positions[i] - (noCols + noDepth);
		}
    }
	
    if (actualCols == 0 || actualDepth == 0 || actualTime == 0)
        return 0;
    ...
    for (int r = 0; r < noRows; r++)
	{
        ...
        for (int c = 0; c < actualCols; c++)
		{
			...
			{
				for (int t = 0; t < actualTime; t++)
				{
					int col = cols[c];
					int dep = depths[d];
					int tim = times[t];
					int idx = r + noRows * col + noRows * noCols * dep + noRows * noCols * noDepth * tim;
					pScore += covTen[idx];
				}
            }
        }
		
		double denom = actualCols * actualDepth * actualTime;
...
void initializePop(long int *pop, int noPop, double *covTen, int noCovs, int noDepth, int noSigs, int noTime, double alpha){
   
   int noGenes = noCovs + noDepth + noTime;
   ...
   while (x<(noPop*100) && index<noPop){
      ...
      r2 = rand() % noDepth;
	  r3 = rand() % noTime;
      int positions[3]= {r1, noCovs + r2, noCovs + noDepth + r3};   
      score=getScore4D(positions,actualGenes,covTen,noSigs,noCovs,noDepth,noTime,alpha);
      //printf("%f %d %d %d\n",score,r1,r2,actualCovs);
      if (score>0){
         pop[r1 * noPop + index] = 1;
         pop[(noCovs + r2) * noPop + index] = 1;
		 pop[(noCovs + noDepth + r3) * noPop + index] = 1;
         index++;
      }
      x++;
   }    
...
void sortPop(long int *pop, int noGenes, int noPop, double *scores){
   ...
   long int *sortedPop = malloc(sizeof(long int) * noGenes * noPop);
...
void clusterCovs4DC(double *covTen,
                    int *group,
                    int *noCovs,
                    int *noSigs,
                    int *noDepth,
                    ...
                    double *SP)
{
    ...
    initializePop(pop, *noPop, covTen, *noCovs, *noDepth, *noSigs, *noTime, *alpha);
    ...
    for (int j = 0; j < *noPop; j++)
    {
        ...
        scores[j] = getScore4D(positions, actualGenes, covTen, *noSigs, *noCovs, *noDepth, *noTime, *alpha);
    }
    ...
    while (stag < *maxStag)
    {
        ...
        generateChildren(pop, covTen, scores, *noPop, noGenes, *noCovs, *noDepth, *noSigs, *noTime, *SR, *max_SP, SP, *mutation, *alpha);
